...
void testGraph(std::shared_ptr<graph> graph_,std::shared_ptr<Node> final) {
    char buffer[64]={0};
    size_t length=64;
    __cxxabiv1::__cxa_demangle(typeid(graphType).name(),buffer,&length,nullptr);
    ...
    for (auto &node:graph_->nodes) {
        auto graphNode=runGraph->add_node(node->func)->name("node"+std::to_string(number++));
    ...
    } else if constexpr(std::is_same<graphType,cppSync::noCoroutine::graph>::value) {
        runGraph->run_graph();
        finalNode->get_promise()->get_future().get();
    } else {
