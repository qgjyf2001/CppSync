...
public:
  ...
  public:
    std::string name_;
    Node(graph *graph_, std::function<void()> func,
         std::string name = "unknown")
        : graph_(graph_), func(func), name_(name) {
    ...
    auto get_promise() { return promise_; }
    void operator()() {
      func();
      promise_->set_value();
...
public:
  ...
  public:
    ...
    void depend(std::shared_ptr<Node> node) { depends.push_back(node); }
    task operator()() {
      for (auto i : depends) {
      ...
      co_await awaiter(cppSync::async(
          [](std::function<void()> &&func) { func(); }, std::move(func)));
      co_return;
    }
  };
